...
void setup() {
  ...
  float raw_analog_read = analogRead(batteryPin);
  float v_in = raw_analog_read * 3.3 / 4095;
  float v_batt = v_in * 1.487;
  
  Serial.print("Battery voltage: ");
  Serial.print(v_batt);
  Serial.println("V");
  
  // Determine operating mode based on battery and wake cause
  if (v_batt >= PLUGGED_IN) {
    Serial.println("Plugged in - Debug mode");
    mode = DEBUG;
  } else if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT0) {
    Serial.println("Button wake - AP mode");
    mode = AP_MODE;
  } else if (v_batt < BATTERY_MIN_VOLTAGE) {
    Serial.println("Battery dead");
    mode = DEAD;
  } else if (v_batt < BATTERY_LOW_VOLTAGE) {
    Serial.println("Low battery - conservation mode");
    mode = LOW_POWER;
  } else {
    Serial.println("Normal battery - normal mode");
    mode = NORMAL;
  }
  ...
  switch(mode) {
    case AP_MODE:
      AP_Mode();
      return; // Don't continue with normal WiFi setup
    case DEAD:
      run_very_low_battery_mode(v_batt);
      enter_deep_sleep();
      return;
    case LOW_POWER:
      run_conservation_mode(v_batt);
      break;
    case DEBUG:
      debug_mode(v_batt);
      break;
    case NORMAL:
    default:
      run_normal_mode(v_batt);
      break;
  }
  
  // Setup WiFi connection for normal operation
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    start_wifi(); // Try to reconnect
    if (WiFi.status() != WL_CONNECTED) {
      Serial.println("Reconnection failed, entering deep sleep");
      enter_deep_sleep();
      return;
    }
  }
...
void start_wifi(){
  ...
  // Re-enable WiFi
  WiFi.mode(WIFI_STA);
  delay(500); // Give time for WiFi radio to initialize
  ...
  const int max_attempts = 40; // Increased timeout to 20 seconds
  ...
  while (WiFi.status() != WL_CONNECTED && connection_attempts < max_attempts) {
    ...
    if(connection_attempts % 5 == 0) {
      ...
      if(connection_attempts % 10 == 0 && connection_attempts < max_attempts) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("2. Check if WiFi network is available");
    Serial.println("3. Check signal strength");
    Serial.println("4. Check power supply (use quality USB cable)");
    
    // Scan for networks to help debug
    Serial.println("Scanning for available networks...");
    int n = WiFi.scanNetworks();
    if (n > 0) {
      for (int i = 0; i < min(n, 5); ++i) { // Show only first 5
        Serial.print("  ");
